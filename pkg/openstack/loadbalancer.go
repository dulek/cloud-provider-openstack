	...
	if monitorID != "" {
		if !svcConf.enableMonitor {
			...
			if createOpts.Type != monitor.Type {
				...
				// We're all good, monitor exists and is of the proper type, let's make sure it has up to date
				...
				// This is the happy path, monitor exists, has correct type and rest of the properties.
				return nil
			}
		}

		// We need to delete the old monitor before it's recreated.
